...
def length_extension_sha1(oldmac, oldmsg, keylen, extension):
    ...
    pad = get_padding( len(oldmsg) + keylen)

    n = 4
    h0, h1, h2, h3, h4 = [oldmac[i:i+n] for i in range(0, 20, n)]

    sha = Sha1Hash()
    sha.set_registers(h0, h1, h2, h3, h4)
    sha._message_byte_length  = len(oldmsg) + len(pad)
    sha._message_byte_length += 64 - (sha._message_byte_length % 64)
    sha._unprocessed = b''

    new_mac = sha.update(extension).hexdigest()
...
def main():
    ...
    new_mac, new_msg = length_extension_sha1(mac, msg, keylen, msg_extension)
